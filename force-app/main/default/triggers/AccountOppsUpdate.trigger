...
//update all the 'Open' opportunities StageName to 'Closed Lost'
    ...
    List<Opportunity> tobeUpdatedOpps = [SELECT Name, AccountId, StageName
                                        FROM Opportunity
                                        WHERE IsClosed = FALSE //WHERE IsWon = FALSE  ile aynı sonucu veriyor isLost diye bir field yoko çalışmıyor IsClosed-IsWon Fieldleri var sonuçları boolean olarak geliyor
